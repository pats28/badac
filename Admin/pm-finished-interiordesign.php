...
if (isset($_POST['btnSearch_project'])) {
    ...
    $query = "SELECT * FROM project as p, client as c, employee as e, desiredspecs as d WHERE d.ProjectId = p.ProjectId AND c.ClientId = p.ClientId AND e.EmpId = p.EmpId AND p.isdelete = 0 AND p.ServiceTypeId = 2 AND CONCAT (p.ProjectName, c.FirstName, c.LastName, e.EmpFirstName, e.EmpLastName) LIKE '%" . $search_project . "%'";
    $search_result = searchTable2($query);
} else {

    $query = "SELECT * FROM project as p, client as c, employee as e, desiredspecs as d, payment as pay WHERE d.ProjectId = p.ProjectId AND  c.ClientId = p.ClientId AND e.EmpId = p.EmpId AND p.isdelete = 0 AND p.ServiceTypeId = 2 AND p.Progress = 100 AND p.ProjectId = pay.ProjectId";
